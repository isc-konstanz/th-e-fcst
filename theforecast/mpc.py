...
class MPC:
    ...
    def execute(self, prediction):
        '''Description: executes the MPC. Calculates the optimal controlling of the device
        :input prediction:
        ...
        try:
            if solver.Solve() == 0:
                ...
                logger.warning('Optimization problem. Solution not found.')
        except:
            raise MPCException()
...
class MPCException(Exception):
    """
    Raise if any error occurred during the optimization process.
